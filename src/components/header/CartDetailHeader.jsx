...
export default () => {
    ...
    const [cartLeftClass, setCartLeftClass] = useState(window.innerWidth > 767);
    useEffect(() => {

        const handlResize = () => {
            const accSize = window.innerWidth
            if (accSize > 767) {
                setCartLeftClass(false);
            } else {
                setCartLeftClass(true);
            }
        }
        handlResize();
    ...
    return (
        ...
        <>
            <section className={`absolute top-[120px] right-0 mt-5 md:top-[128px] mx-[5%]  max-w-auto md:px-[350px] md:max-w-[600px] md:-mt-10 ${window.innerWidth < 768 ? "left-0" : ""}`}>
                <div className="mx-4 shadow-xl bg-white rounded-xl border border-gray-300 md:w-[350px]">
                    ...
                    {
                        cartProducts.length === 0 && <p className=" py-8 pb-[50px] mt-8 font-bold text-center text-gray-600 px-20 md:px-[100px]">Your cart is empty</p>
                    }
                    {
                        cartProducts.map(product => (

                            <article key={product.id} className="px-6 gap-3 sm:gap-6 md:gap-1 py-4 grid grid-cols-[1fr_1fr_0.5fr] sm:grid sm:grid-cols-[1fr_1.5fr_1fr] md:grid-cols-[2fr_4fr_1fr] p-10 items-center">
